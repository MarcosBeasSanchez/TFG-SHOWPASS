...
public class ServicioCarritoImpl implements IServicioCarrito{
	 ...
	 @Autowired  RepositorioCarrito carritoDAO;
	    ...
	    @Override
	    public DTOCarritoBajada agregarEventoPorCarrito(Long carritoId, Long eventoId, int cantidad) {
	 ...
	 @Override
	 public DTOCarritoBajada finalizarCompraPorCarrito(Long carritoId) {
	     Carrito carrito = carritoDAO.findById(carritoId)
	             .orElseThrow(() -> new RuntimeException("Carrito no encontrado"));
	     ...
	     // Vaciar carrito sin cambiar el estado
	     carrito.getItems().clear();
	     // carrito.setEstado(EstadoCarrito.FINALIZADO); <-- eliminar esta línea
	     carritoDAO.save(carrito);
